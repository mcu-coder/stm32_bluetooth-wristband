...
extern unsigned char BUF[6];                         //接收数据缓存区 


unsigned char ADXL345_Init(void); 					//初始化ADXL345
void ADXL345_WR_Reg(unsigned char addr,unsigned char val);		//写ADXL345寄存器
unsigned char ADXL345_RD_Reg(unsigned char addr);				//读ADXL345寄存器
void ADXL345_RD_XYZ(unsigned char *pBuffer);
void ADXL345_RD_Avval(short *x,short *y,short *z);				//读取平均值
void ADXL345_Write_Single(unsigned char REG_Address,unsigned char REG_data);	//单字节写入
